...
#define		BLINK_LED_WHILE_WAITING
	...
	#define PROGLED_DDR		DDRB
	#define PROGLED_PIN		PINB7
#elif defined( _BOARD_256RFR2XPRO_ )
	#define PROGLED_PORT	PORTB
	#define PROGLED_DDR		DDRB
	#define PROGLED_PIN		PINB4
	#define PROGSW_PORT	    PORTE
	#define PROGSW_DDR		DDRE
	#define PROGSW_PIN		PINE4
	#define PROGSW_PIN_REG  PINE
#elif defined( _BOARD_256RFR2ZBITXPRO_ )
	#define PROGLED_PORT	PORTE
	#define PROGLED_DDR		DDRE
	#define PROGLED_PIN		PINE2
	#define PROGSW_PORT	    PORTE
	#define PROGSW_DDR		DDRE
	#define PROGSW_PIN		PINE0
	#define PROGSW_PIN_REG  PINE	
#else
...
#elif defined(__AVR_ATmega256RFR2__) && ((_BOARD_256RFR2XPRO_) || _BOARD_256RFR2ZBITXPRO_ )
	/* ATMega with two USART, use UART0 */
	#define	UART_BAUD_RATE_LOW			UBRR1L
	#define	UART_STATUS_REG				UCSR1A
	#define	UART_CONTROL_REG			UCSR1B
...
//	boot_timeout	=	170000;
	boot_timeout	=	 150000;		//*	should be about 15 second
#else
...
//	PROGLED_PORT	&=	~(1<<PROGLED_PIN);	// active low LED ON
	 ...
	 */
	 // Clear Uart status reg and flush out the previous data in startup
	 UART_STATUS_REG	= 0x00;
	 UART_CONTROL_REG	=	0x00;
	 c			=	UART_DATA_REG;	 
...
//	sendchar('0');
    ...
    io_state = PROGSW_PIN_REG & (1<<PROGSW_PIN);
	while (boot_state==0 && io_state == 0x00)
	{
